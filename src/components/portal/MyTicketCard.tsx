...
export function MyTicketCard({ ticket }: { ticket: Ticket }) {
  return (
    <Link href={`/my-tickets/${ticket.id}`} className="panel block rounded-[2rem] p-6 transition hover:-translate-y-0.5 hover:border-cyan-400/20 hover:bg-white/[0.06]">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-sm font-medium text-cyan-200">{ticket.id}</p>
          <h3 className="mt-3 text-xl font-semibold text-white">{ticket.summary}</h3>
        </div>
